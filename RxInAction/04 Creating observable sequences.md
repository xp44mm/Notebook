...
The following listing is an example of an observer that will accompany us throughout the chapter. This observer writes to the console the OnNext, OnComplete, and OnError actions as they happen.
...
type ConsoleObserver<'T> (name:string) =
    interface IObserver<'T> with
        member _.OnNext(value:'T) =
            Console.WriteLine("{0} - OnNext({1})",name,value)
...
let subscription =
    (numbers:>IObservable<int>).Subscribe(ConsoleObserver<int>("numbers"):>IObserver<_>);
...
It's much nicer to consume the chat messages with an observable. The mapping, shown in figure 4.2, is clear between the events and what the observer knows to handle:
  ...
  * Closed event can be mapped to the observers’ OnComplete
...
去掉多余的`ChatClient`类型，精简整理后的F#代码：
...
```F#
let wifiScanner = new WifiScanner1();
let networks =
    Observable.FromEvent<NetworkFoundEventHandler, string>(
...
class WifiScanner
{
    public event ExtendedNetworkFoundEventHandler ExtendedNetworkFound =
delegate { };
}
```
...
```F#
let wifiScanner = new WifiScanner2();
let networks =
    let conversion (rxHandler:Action<string*int>) =
         ExtendedNetworkFoundEventHandler(fun ssid strength -> rxHandler.Invoke(ssid, trength))
...
networks.Subscribe(ConsoleObserver "ExtendedNetworkFound") |> ignore
...
F#
```F#
let wifiScanner = new WifiScanner0()
let networks =
    Observable.FromEvent(
        (fun h -> wifiScanner.Connected.AddHandler h),
        (fun h -> wifiScanner.Connected.RemoveHandler h))
networks.Subscribe(ConsoleObserver "Connected") |> ignore
...
Under the hood, the `ToObservable` method creates an observable that, once subscribed into, iterates on the collection and passes each item to the observer. When the iteration is done, the `OnComplete` method is called on the observer.
...
IObservable<string> liveMessages   =
