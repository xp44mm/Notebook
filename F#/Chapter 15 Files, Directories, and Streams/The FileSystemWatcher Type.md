...
You can create a FileSystemWatcher component in either of  two ways: by means of code or by dragging it from the Components tab of the  Toolbox to the tray area of a Windows Forms class, a Web Forms page, or another  Microsoft Visual Studio designer. There's no noticeable difference in  performance or flexibility, so any method is fine. The demo application uses a  component in a form's component tray area, which I have renamed fsw (see [Figure 15-1](#ch15fig01)), but creating it through  code is equally simple:
...
By default, the Created, Deleted, Renamed, and Changed  events run in a thread taken from the system thread pool. (See [Chapter 20](BBL0103.html#1638),  "Threads," for more information about the thread pool.) Because Windows Forms  controls aren't thread safe, you should avoid accessing any control or the form  itself from inside the FileSystemWatcher component's event handlers. If you find  this limitation unacceptable, you should assign a Windows Forms control to the  component's SynchronizingObject property, as in this code:
