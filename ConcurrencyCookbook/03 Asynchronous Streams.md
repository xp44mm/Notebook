...
async IAsyncEnumerable<string> GetValuesAsync(HttpClient client)
{
  ...
  while (true)
  {
    ...
    string result = await client.GetStringAsync(
        $"https://example.com/api/values?offset={offset}&limit=
{limit}");
...
int count = await SlowRange().CountAsync(
    value => value % 2 == 0);
That predicate can also be asynchronous, in which case you would then use the
CountAwaitAsync operator, since it both takes an asynchronous delegate
(which it will await) and produces a single terminal value, the count:
