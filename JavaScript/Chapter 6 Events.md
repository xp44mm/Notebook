...
All of the list items are contained within an unordered list tag, so why not take advantage of the fact that we can capture click events at the <ul> level? The only thing we need is some way to differentiate between the various list items. Back in the section on traditional event binding, when we discussed the this object, we noted that this refers to the element where the event is captured, while event.target refers to the element that actually emits the event in question. Clearly, we could use a combination of this and event.target.

But the event-handling specification provides the event.currentTarget property to solve this problem.
