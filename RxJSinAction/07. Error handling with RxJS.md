...
同步代码用try/catch实现错误处理。异步代码用承诺实现错误处理。
...
retry最好用在merge后面，因为Promise不可变，需要重新建。
...
of(2, 4, 5, 8, 10)
    ...
    |> retryWhen(
        errors$ =>
            zip(range(0, maxRetries + 1), errors$, (i, err) => ({ i, err }))
            |> mergeMap(({ i, err }) => {
                if (i < maxRetries) {
                    return (
                        timer(i * 1000)
    ...
    |> subscribe(v => console.log(v), error => console.log(error.message))
```
