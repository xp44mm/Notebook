...
> A long time ago I switched the names there from `observer` to `subscriber`, because truly it's not *just* an observer. For example, you can call `unsubscribe` on it. Or you can add teardown logic to the subscription. It's both an "observer" and a "subscription". It's a "subscriber".
