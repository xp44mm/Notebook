...
`querySelectorAll`返回一个`NodeList`集合。（即使它看起来像一个数组）
...
`event` 是你想要监听的事件的名字。这些事件在参数里已经被预先确定了。
...
我们可以用事件对象（e）做很多事情。但那是另外的话题。让我们继续移除事件监听。
...
1. 当用户点击一个按钮的时候添加 `.is-open` 
2. 当用户点击按钮的时候移除 `.is-open`
3. `<>`的过渡已经用css做了。
...
在上面如果你多次点击这个按钮，你会看到我像添加其它列表项一样添加了文本`Hello again, world!`到这个DOM里。
...
作为一个前端开发者，修改DOM是你需要知道的最重要的事情之一，你可以用DOM做各种你想象到的事情。 在这篇文章中，我向你展示了五个你需要改变DOM常用的方法，附加你需要了解的关键代码。现在，来吧和DOM玩耍，来创造奇迹吧:)